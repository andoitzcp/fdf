...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/14 00:33:48 by andoitzcp         #+#    #+#             */
/*   Updated: 2025/02/14 02:26:39 by andoitzcp   ########  ###                */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

int	ft_calcheigh(t_node **head)
{
	unsigned int	i;
	unsigned int	j;
	t_node			*p;
...
t_node	*ft_defpoint(int x, int y, char *param)
{
	...
	p->z = ft_atoi(handc[0]);
	if (!handc[1])
		p->c = MIN_COLOR + p->z * (0xFF0000 - MIN_COLOR);
	else
		p->c = ft_xtoi(handc[1]);
